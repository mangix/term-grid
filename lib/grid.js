...
Grid.prototype = {
     ...
     * append row
     * @param data{Array}
     * @param drawThisRow{Boolean} ,  draw this row immediately
     * */
    appendRow: function (data, drawThisRow) {
        if (this._validRow(data)) {
            this.data.push(data);
            if (drawThisRow) {
                this.draw([this._compileRow(this.data.length - 1)]);
            }
        }
    },
    /**
    ...
    compile: function () {
        ...
        this.data.forEach(function (row, i) {
            data.push(this._compileRow(i, columnWidthCache));
        }, this);
        return data;
    },
     ...
     * draw
     * */
    draw: function (specialData) {
        var data = specialData || this.compile();
        data.forEach(function (row) {
     ...
     * compile row
     * */
    _compileRow: function (rowIndex, columnWidthCache) {
        ...
        row.forEach(function (column, j) {
            ...
            rowData.push(this._parseColor(rowIndex, j, cell(column.toString(), columnWidthCache[j], this.align[j])));
        }, this);
        return rowData.join("");
    },
